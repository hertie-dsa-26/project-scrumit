...
class MoneyLaunderingPredictor:
    ...
    def preprocess_transaction(self, raw_data: dict) -> np.ndarray:
        ...
        Args:
            raw_data: Dictionary with transaction features (should already be imputed)
        ...
        # Expected features (all should be present after imputation)
        expected_features = [
        ...
        df_raw = pd.DataFrame([{f: raw_data.get(f) for f in expected_features}])
    ...
    def predict(self, raw_data: dict) -> dict:
        """
        Make prediction on raw transaction data with automatic imputation.

        Args:
            raw_data: Dictionary with transaction features (missing values will be imputed)
        ...
        Returns:
            Dictionary with prediction results:
            {
                ...
                'imputed_fields': list (fields that were imputed with defaults)
            }
        """
        # Impute missing values with defaults
        imputed_data = FeatureImputationService.impute_missing(raw_data)

        # Track which fields were imputed
        imputed_fields = [
            k for k in imputed_data.keys()
            if k not in raw_data or raw_data[k] is None or raw_data[k] == ''
        ]

        # Preprocess
        features = self.preprocess_transaction(imputed_data)
